  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full bg-[url('/bg.png')] bg-cover bg-center text-white p-6">
      <div className="mb-8 flex flex-col items-center">
        <img src="/elbaFinal.png" alt="Elba Logo" className="mb-12" />
        <h1 className="text-[70px] font-bold mb-2 font-heading text-center">{correctAnswers} / {totalQuestions}</h1>
        {/* <p className={`text-2xl font-bold mb-4 ${color}`}>
          {correctAnswers} de {totalQuestions} ({percentage}%)
        </p> */}
        <p className="text-4xl mb-8 text-center font-heading p-8">{preventWidows(message)}</p>
      </div>

      <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md justify-center">


        <img onClick={onRestart} src="/restart.png" alt="Recomeçar" />



        <img onClick={onCuriosity} src="/btnCuriosidades.png" alt="Curiosidades" />
      </div>
    </div>
  )
}
